...
public class ElasticSearchRestHighImpl implements ElasticSearchService {
  ...
  static Logger logger= LoggerFactory.getLogger(ElasticSearchRestHighImpl.class);
  ...
  public Future<String> save(String index, String identifier, Map<String, Object> data) {
    ...
    logger.info("ElasticSearchUtilRest:save: method started at ==" + startTime + " for Index " + index);
    ...
    if (searchDTO.getSortBy() != null && searchDTO.getSortBy().size() > 0) {
      for (Map.Entry<String, Object> entry : searchDTO.getSortBy().entrySet()) {
        if (!entry.getKey().contains(".")) {
          ...
          Map<String, String> dataMap = (Map) map.get(EsJsonKey.TERM);
          for (Map.Entry<String, String> dateMapEntry : dataMap.entrySet()) {
            FieldSortBuilder mySort =
                new FieldSortBuilder(entry.getKey() + ElasticSearchHelper.RAW_APPEND)
                    ...
                    .order(ElasticSearchHelper.getSortOrder((String) map.get(EsJsonKey.ORDER)));
        ...
        new ActionListener<SearchResponse>() {
          ...
          public void onResponse(SearchResponse response) {
            ...
            if (response.getHits() == null || response.getHits().getTotalHits() == 0) {
              ...
              responseMap.put(EsJsonKey.COUNT, 0);
  ...
  public Future<Boolean> bulkInsert(String index, List<Map<String, Object>> dataList) {
    ...
    for (Map<String, Object> data : dataList) {
      request.add(new IndexRequest(index, _DOC, (String) data.get(EsJsonKey.ID)).source(data));
    }
    ActionListener<BulkResponse> listener =
        new ActionListener<BulkResponse>() {
          ...
          @Override
          public void onFailure(Exception e) {
            logger.info("ElasticSearchRestHighImpl:bulkinsert: Bulk upload error block", e);
            promise.success(false);
          }
        };
  ...
  public Future<Map<String, Map<String, Object>>> getEsResultByListOfIds(
    ...
    filters.put(EsJsonKey.ID, ids);

    SearchDTO searchDTO = new SearchDTO();
    searchDTO.getAdditionalProperties().put(EsJsonKey.FILTERS, filters);
    searchDTO.setFields(fields);

    Future<Map<String, Object>> resultF = search(searchDTO, index);
    Map<String, Object> result =
        (Map<String, Object>) ElasticSearchHelper.getResponseFromFuture(resultF);
    List<Map<String, Object>> esContent = (List<Map<String, Object>>) result.get(EsJsonKey.CONTENT);
